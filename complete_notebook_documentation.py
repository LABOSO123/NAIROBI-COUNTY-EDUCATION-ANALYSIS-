"""
Complete notebook documentation enhancement:
1. Fix introduction formatting
2. Add comprehensive documentation to ALL sections
3. Add code comments
...
print("="*80)
print("COMPLETE NOTEBOOK DOCUMENTATION ENHANCEMENT")
print("="*80)
print()
...
# Fix introduction cell formatting
print("1. Fixing introduction cell formatting...")
for i, cell in enumerate(nb['cells']):
    if cell.get('cell_type') == 'markdown':
        source = ''.join(cell.get('source', []))
        if '# Nairobi Primary Education Equity Analysis' in source and '## Multi-Dimensional' in source:
            # The content is there but needs proper line breaks
            # Reconstruct with proper formatting
            new_source = """# Nairobi Primary Education Equity Analysis
...
"""
            ...
            print(f"   ✅ Fixed introduction cell at index {i}")
            break
...
section_docs = {
    '3': {
        ...
        'content': """**What This Section Does:**
...
**Sub-Analyses:**
1. **School Density vs Population Density**: Calculates schools per 1000 school-age children by geographic unit
...
3. Where are the "education deserts"?
...
- Girls' toilet counts
- Population gender breakdown (if available)

**Key Metrics Calculated:**
- Gender enrollment percentage
- Gender imbalance flag (difference > 15%)
- Girls' toilet ratio
- `Girls_Toilet_Deficit`: Binary indicator
...
- School-age % = ~16% of total population
...
**Datasets Used:**
- All previous analyses aggregated by geographic unit

**Key Metrics Calculated:**
- `School_Density_Score`: Based on schools per 1000
...
2. What's the overall priority ranking?
...
print("\n2. Updating section headers with comprehensive documentation...")
updated_sections = []
...
for i, cell in enumerate(nb['cells']):
    if cell.get('cell_type') == 'markdown':
        ...
        for num, doc in section_docs.items():
            # Look for section number in various formats
            patterns = [
                rf'##\s*{num}\.',
                rf'##\s*{num}\s+',
                rf'#{num}\.'
            ...
            for pattern in patterns:
                if re.search(pattern, source, re.IGNORECASE):
                    ...
                    if 'What This Section Does' not in source:
                        ...
                        nb['cells'][i]['source'] = new_source.split('\n')
                        updated_sections.append(num)
                        print(f"   ✅ Updated section {num}")
                        break

print(f"\n   Updated {len(updated_sections)} sections: {', '.join(updated_sections)}")

# Add code comments to key code cells
print("\n3. Adding code comments...")
...
for i, cell in enumerate(nb['cells']):
    if cell.get('cell_type') == 'code':
        ...
        # Add comments to data loading cells
        if 'pd.read_csv' in source and '# Load' not in source:
            # Check if it's a data loading cell
            if 'kenya_primary_schools' in source or 'population' in source.lower():
                # Add comment at the beginning if not present
                if not any('# Load' in line or '# This dataset' in line for line in source_lines[:3]):
                    new_lines = ['# Load dataset into pandas DataFrame\n', '# This dataset contains...\n'] + source_lines
                    cell['source'] = new_lines
                    code_comment_count += 1

print(f"   ✅ Added comments to {code_comment_count} code cells")

print("\n" + "="*80)
print("✅ NOTEBOOK ENHANCEMENT COMPLETE")
print("="*80)
print(f"   - Fixed introduction formatting")
print(f"   - Updated {len(updated_sections)} section headers")
print(f"   - Added comments to {code_comment_count} code cells")
print("="*80)
